...
namespace Streets
{
    public partial class StreetsForm : Form
    {
        ...
        {
            string qwertystring = $"select * from Rights where ID_Menu = 17";
        ...
        {
            dgw.Rows.Clear();
            string qwertystring = $"select * from Улица";
            OleDbCommand command = new OleDbCommand(qwertystring, b.getConnection());
            b.openConnection();
            OleDbDataReader reader = command.ExecuteReader();
            while (reader.Read())
        ...
        {
            AddStreets addfrm = new AddStreets();
            addfrm.Show();
        }
        ...
        {
            dgw.Rows.Clear();
            string serchString = $"select * from Улица where Наименование like '%" + textBox_search.Text + "%'" + " or ID like '%" + textBox_search.Text + "%'";
        ...
        private void Update()
        {
            ...
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                ...
                if (rowState == RowState.Deleted)
                {
                    ...
                    var deleteQwery = $"delete from Улица where ID = {id}";
                ...
                if (rowState == RowState.Modifide)
                {
                    ...
                    var changeQwery = $"update Улица set Наименование = '{name}' where ID ={id}";
