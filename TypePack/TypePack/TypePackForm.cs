...
namespace TypePack
{
    public partial class TypePackForm : Form
    {
        ...
        // Авторизация прав пользователя на меню ВидУпаковки.
        private void RightsFunction()
        {
            string qwertystring = $"select * from Rights where ID_Menu = 19";
        ...
        private void TypePackForm_Load(object sender, EventArgs e)
        {
            CreateColums();
        ...
        {
            dgw.Rows.Clear();
            string qwertystring = $"select * from ВидУпаковки";
            OleDbCommand command = new OleDbCommand(qwertystring, b.getConnection());
            b.openConnection();
            OleDbDataReader reader = command.ExecuteReader();
            while (reader.Read())
        ...
        {
            AddTypePack addfrm = new AddTypePack();
            addfrm.Show();
        }
        ...
        {
            dgw.Rows.Clear();
            string serchString = $"select * from ВидУпаковки where Наименование like '%" + textBox_search.Text + "%'" + " or ID like '%" + textBox_search.Text + "%'";
        ...
        private void Update()
        {
            ...
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                ...
                if (rowState == RowState.Deleted)
                {
                    ...
                    var deleteQwery = $"delete from ВидУпаковки where ID = {id}";
                ...
                if (rowState == RowState.Modifide)
                {
                    ...
                    var changeQwery = $"update ВидУпаковки set Наименование = '{name}' where ID ={id}";
