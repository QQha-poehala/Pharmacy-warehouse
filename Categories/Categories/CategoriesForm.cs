...
namespace Categories
{
    public partial class CategoriesForm : Form
    {
        ...
        // Авторизация прав пользователя на меню Катагоря.
        private void RightsFunction()
        {
            string qwertystring = $"select * from Rights where ID_Menu = 18";
        ...
        {
            dgw.Rows.Clear();
            string qwertystring = $"select * from Категория";
            OleDbCommand command = new OleDbCommand(qwertystring, b.getConnection());
            b.openConnection();
            OleDbDataReader reader = command.ExecuteReader();
            while (reader.Read())
        ...
        {
            AddCategories addfrm = new AddCategories();
            addfrm.Show();
        }
        ...
        {
            dgw.Rows.Clear();
            string serchString = $"select * from Категория where Наименование like '%" + textBox_search.Text + "%'" + " or ID like '%" + textBox_search.Text + "%'";
        ...
        private void Update()
        {
            ...
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                ...
                if (rowState == RowState.Deleted)
                {
                    ...
                    var deleteQwery = $"delete from Категория where ID = {id}";
                ...
                if (rowState == RowState.Modifide)
                {
                    ...
                    var changeQwery = $"update Категория set Наименование = '{name}' where ID ={id}";
