...
namespace Order
{
    public partial class AddLekZak : Form
    {
        DataB database = new DataB();
        public int id_Zak;
        public int id_User;
        public AddLekZak(int id_user, int id_zak, DataGridView dgw)
        {
            id_User = id_user;
            id_Zak = id_zak;
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (lek_id>=0 )
            {
                if(kol <= kol_in_kat)
                {
                    addQwery = $"insert into `Счёт-фактуры` (Лекарство_ID, Количество, Заказ_ID, Статус) values ({lek_id}, {kol}, {id_Zak}, 'Добавлен')";
                ...
                {
                    MessageBox.Show("Такого количества нет на складе!", "Создание записи", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            else
            {
