...
namespace Buyers
{
    public partial class AddBuyers : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (isNumber1 == true && isNumber3 == true && tel.Length <=12 && inn.Length <=20 &&house > 0 && kvar > 0 && name != "" && streets_id > 0)
            {
                if (isNumber2 == false)
                {
                    addQwery = $"insert into Покупатель (Название, Улица_ID, Дом, Строение, Квартира, Телефон, ИНН) values ('{name}', {streets_id}, {house}, NULL, {kvar}, '{tel}', '{inn}')";
                }
                else
                {
                    addQwery = $"insert into Покупатель (Название, Улица_ID, Дом, Строение, Квартира,Телефон, ИНН) values ('{name}', {streets_id}, {house}, {stroen}, {kvar},'{tel}', '{inn}' )";
                }
