...
namespace Buyers
{
    public partial class BuyersForm : Form
    {
        ...
        {
            string qwertystring = $"select * from Rights where ID_Menu = 10";
        ...
        private void ReadSingleRow(DataGridView dgw, IDataRecord record)
        {
            ...
            dgw.Rows.Add(record.GetInt32(0), record.GetString(1).Trim(), record.GetInt32(2), record.GetInt32(3), srtoen, record.GetInt32(5), record.GetString(6).Trim(), record.GetString(7).Trim(), RowState.ModifideNew);
        }
        // Поиск данных в базе данных.
        private void RefreshDataGrid(DataGridView dgw)
        {
            dgw.Rows.Clear();
            string qwertystring = $"select * from Покупатель";
            OleDbCommand command = new OleDbCommand(qwertystring, b.getConnection());
            b.openConnection();
            OleDbDataReader reader = command.ExecuteReader();
            while (reader.Read())
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                textBox7.Text = row.Cells[7].Value.ToString().Trim();
        ...
        {
            AddBuyers addfrm = new AddBuyers();
            addfrm.Show();
        }
        ...
        {
            dgw.Rows.Clear();
            string serchString = $"select * from Покупатель where ID like '%" + textBox_search.Text + "%'" + " or Название like '%" + textBox_search.Text + "%'" + " or Улица_ID like '%" + textBox_search.Text + "%'" + " or Дом like '%" + textBox_search.Text + "%'" + " or Строение like '%" + textBox_search.Text + "%'" + " or Квартира like '%" + textBox_search.Text + "%'" + " or Телефон like '%" + textBox_search.Text + "%'" + " or ИНН like '%" + textBox_search.Text + "%'";
        ...
        private void Update()
        {
            ...
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                ...
                if (rowState == RowState.Deleted)
                {
                    ...
                    var deleteQwery = $"delete from Покупатель where ID = {id}";
                ...
                if (rowState == RowState.Modifide)
                {
                    ...
                    var tel = dataGridView1.Rows[i].Cells[6].Value.ToString().Trim();
                    var inn = dataGridView1.Rows[i].Cells[7].Value.ToString().Trim();
                    string changeQwery;
                    if (stroen != 0)
                    {
                        changeQwery = $"update Покупатель set Название = '{name}', Улица_ID ={streets_id}, Дом = {house}, Строение= {stroen}, Квартира= {kvar}, Телефон= '{tel}', ИНН= '{inn}' where ID ={id}";
                    }
                    else
                    {
                        changeQwery = $"update Покупатель set Название = '{name}', Улица_ID ={streets_id}, Дом = {house}, Строение= NULL, Квартира= {kvar}, Телефон= '{tel}', ИНН= '{inn}' where ID ={id}";
                    }
        ...
        private void Change()
        {
            ...
            // Проверка ввода.
            if (isNumber1 == true && isNumber3 == true && tel.Length <=12 && inn.Length <=20 && name != "" && Convert.ToInt32(house) > 0 && Convert.ToInt32(kvar) > 0)
            {
                int strorn_int;
                bool isNumber2 = int.TryParse(textBox4.Text, out strorn_int);
                if (isNumber2 == true)
                {
                    dataGridView1.Rows[selectedRowindex].SetValues(id, name, streets_id, house, strorn_int, kvar, tel, inn);
                }
                else
                {
                    dataGridView1.Rows[selectedRowindex].SetValues(id, name, streets_id, house, 0, kvar, tel, inn);
                }
                // установка статуса: Измененная строка.
                dataGridView1.Rows[selectedRowindex].Cells[8].Value = RowState.Modifide;
